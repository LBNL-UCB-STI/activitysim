...
def calibrate(config_file, args):
    ...
    try:
        ...
        print(f"Error: Calibration config file not found at {config_file}.", file=sys.stderr)
        sys.exit(1) # Exit if calibration config is missing
    except Exception as e:
        print(f"Error loading calibration config file {config_file}: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if not main_settings_file_name:
        print("Error: 'main_settings_file' (filename relative to data_dir) not specified in calibration config.", file=sys.stderr)
        sys.exit(1)
    ...
    def objective(x):
        ...
        main_settings_file_path = os.path.join(data_base_dir, main_settings_file_name)
        calibration_data_file_path = os.path.join(data_base_dir, calibration_data_file_name)
        ...
        if counter in sample_size_schedule: # Use the sample_size_schedule derived earlier
    ...
    def calculate_errors(model_output_dir, calibration_data_file_path):
        ...
        if not os.path.exists(simulation_output_path):
            print(f"Warning: Simulation output file not found at {simulation_output_path} after iteration {counter}. Returning zero errors.", file=sys.stderr)
            ...
            return np.zeros(_inferred_num_errors if _inferred_num_errors > 0 else expected_shape).flatten()
        ...
        try:
            ...
            if 'distance_bin' not in simulation_output.columns:
                 print(f"Warning: Simulation output file {simulation_output_path} is missing 'distance_bin' column. Cannot calculate errors. Returning zero errors.", file=sys.stderr)
                 expected_shape = calibration_data.shape
                 return np.zeros(_inferred_num_errors if _inferred_num_errors > 0 else expected_shape).flatten()
            ...
            expected_cal_cols = ['HOV', 'Non-Motorized', 'Ride Hail', 'SOV', 'Transit'] # Redefine or pass in if needed
            if 'distance_bin' in calibration_data.columns: # Check if index is set
                 expected_cal_cols = calibration_data.columns # Use columns from loaded calibration data

            sim_output_aligned = simulation_output.reindex(columns=expected_cal_cols, fill_value=0.0)
            aligned_simulation_output = sim_output_aligned.reindex(index=calibration_data.index, fill_value=0.0)
            aligned_calibration_data = calibration_data # Calibration data is the source of truth for shape

        except Exception as e:
             print(f"Error loading simulation output from {simulation_output_path}: {e}. Returning zero errors.", file=sys.stderr)
        ...
        if not os.path.exists(population_count_path):
             print(f"Warning: Population count file not found at {population_count_path} after iteration {counter}. Cannot normalize. Returning zero errors.", file=sys.stderr)
             return np.zeros(aligned_calibration_data.shape).flatten()
        ...
        try:
            ...
            if pop_df.empty or pop_df.shape[0] == 0 or pop_df.shape[1] == 0:
                print(f"Warning: Population count file {population_count_path} is empty or malformed. Cannot normalize. Returning zero errors.", file=sys.stderr)
                return np.zeros(aligned_calibration_data.shape).flatten()
            print(pop_df)
            total_population = pop_df.values[0][0] # Get the first cell value

            if total_population == 0:
                 print(f"Warning: Total population is zero in {population_count_path}. Cannot normalize. Returning zero errors.", file=sys.stderr)
                 return np.zeros(aligned_calibration_data.shape).flatten()

            print(f"Total population loaded from {population_count_path}: {total_population}")
        except Exception as e:
             print(f"Error loading total population from {population_count_path}: {e}. Returning zero errors.", file=sys.stderr)
             return np.zeros(aligned_calibration_data.shape).flatten()
    ...
    def update_sample_size(main_settings_file_path, new_sample_size):
        ...
        try:
            ...
            if settings is None: settings = {} # Handle empty file case
            settings["households_sample_size"] = new_sample_size
            with open(main_settings_file_path, "w") as f:
                yaml.dump(settings, f, default_flow_style=False)
        except FileNotFoundError:
            print(f"Error: settings file not found at {main_settings_file_path}", file=sys.stderr)
        except Exception as e:
            print(f"Error updating settings file {main_settings_file_path}: {e}", file=sys.stderr)
    ...
    def update_coefficients(x, configs_base_dir, tunable_coefficients_config):
        ...
        for coef_file_name, coef_details in tunable_coefficients_config.items():
            file_loc = os.path.join(configs_base_dir, coef_file_name) # Use configs_base_dir passed in
            try:
                with open(file_loc, "r") as f: all_lines = f.readlines()
                new_lines = []
                        ...
                        if line.strip().startswith(f"{coef_name},"):
                            if i < len(x):
                                ...
                                if len(parts) > 1:
                                    ...
                                    original_trailing = parts[1][len(parts[1].rstrip()):]
                                    parts[1] = str(new_value) + original_trailing
                                    modified_line = ",".join(parts)
                                    modified_any = True # Add flag
                                    i += 1
                                else:
                                    print(f"  - Warning: Coefficient line for '{coef_name}' in {file_loc} doesn't seem to have a value part: '{line.strip()}'", file=sys.stderr)
                                break
                            else: # Not enough values in x
                                print(f"Error: More coefficients expected than provided by optimizer x.", file=sys.stderr)
                                break # Stop processing this file's coefs
                    new_lines.append(modified_line)
                if modified_any:
                    with open(file_loc, "w") as f: f.writelines(new_lines)
            except FileNotFoundError:
                 print(f"Error: Coefficient file not found at {file_loc}. Skipping updates for this file.", file=sys.stderr)
                 i += len(coef_details) # Skip indices for this file's coefs
            except Exception as e:
                 print(f"Error processing coefficient file {file_loc}: {e}. Skipping updates for this file.", file=sys.stderr)
                 i += len(coef_details) # Skip indices for this file's coefs
    ...
    try:
        ...
        calibration_data_file_path = os.path.join(data_base_dir, calibration_data_file_name) # Re-construct here for log setup

        num_errors = 0
        try:
            # Temporarily read just to get shape, assuming format is consistent and file exists
            dummy_cal_data = pd.read_csv(calibration_data_file_path).set_index("distance_bin", drop=True)
            ...
            print(f"Inferred {num_errors} error values from calibration data shape.")
             ...
             print(f"Warning: Could not determine expected number of errors from calibration data file {calibration_data_file_path} ({e}). Cannot create full log header with Error_N columns.", file=sys.stderr)
    ...
    for coef_file, coef_details in tunable_coefficients_config.items():
        for coef_name, values in coef_details.items():
            if "initial_value" not in values or "bounds" not in values or not isinstance(values["bounds"], list) or len(values["bounds"]) != 2:
                 print(f"Error: Missing 'initial_value' or 'bounds' for coefficient '{coef_name}' in file '{coef_file}'. Skipping.", file=sys.stderr)
                 continue
    ...
    try:
        result = dfols.solve(objective, x0, bounds=bounds)
...
if __name__ == "__main__":
    ...
    add_run_args(parser) # Adds -c, -d, -o, etc.
    parser.add_argument(
