...
def process_mandatory_tours(persons, mandatory_tour_frequency_alts):
    ...
    if bad_work_dest.sum() > 0:
        logger.warning(
            "At the start we have {0} bad work tour destinations and {1} good ones. Making bad ones stay in same location".format(
                bad_work_dest.sum(), (~bad_work_dest).sum()))
        tours.loc[tours_merged.tour_type == 'work', "destination"].loc[bad_work_dest] = \
            tours.loc[tours_merged.tour_type == 'work', "origin"].loc[bad_work_dest].values
    ...
    if bad_school_dest.sum() > 0:
        ...
        tours.loc[tours_merged.tour_type == 'school', "destination"].loc[bad_school_dest] = \
            tours.loc[tours_merged.tour_type == 'school', "origin"].loc[bad_school_dest].values
