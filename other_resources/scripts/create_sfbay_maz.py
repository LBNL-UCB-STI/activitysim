...
def divideTaz(tazIdx, mazIdxs):
    ...
    maz_sub = joined_sub.loc[mazIdxs].copy()
    maz_sub["pop_probs"] = maz_sub['Population'] / maz_sub['Population'].sum()
    maz_sub["area_probs"] = maz_sub['area_acres'] / maz_sub['area_acres'].sum()
...
out = []
for tazId, mazIds in joined_sub.groupby('taz1454').groups.items():
...
lu_full = pd.concat([new_lu, lu_missing])
...
lu_full['access_dist_transit'] = dist_to_nearest_stop
for col in pop_columns:
    lu_full[col] = lu_full[col].astype(int)

lu_full.drop(columns=['TAZ'], inplace=True)
lu_full['TAZ'] = lu_full['taz1454'].astype(int)
